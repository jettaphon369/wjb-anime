...
class HeroUnit{
  ...
  update(dt,dtms,board){
    ...
    // Face the current enemy target. If this lane has no target, face the nearest enemy.
    ...
    if(!faceTarget){
      ...
      for(const e of board.enemies){
        if(e.dead) continue;
        const dx=e.col-this.col;
        const dz=e.row-this.row;
        const dist=dx*dx+dz*dz;
        if(dist<nearest){ nearest=dist; faceTarget=e; }
      }
    }
    if(faceTarget){
...
class EnemyUnit{
  ...
  update(dt,dtms,board){
    ...
    // Face the closest living hero. Prefer a hero in the same lane;
    // otherwise face the nearest hero anywhere on the battlefield.
    let faceTarget=null;
    let faceDist=Infinity;
    for(const h of board.heroes){
      if(h.dead) continue;
      const lanePenalty=(h.row===this.row)?0:3.5;
      const dx=h.col-this.col;
      const dz=h.row-this.row;
      const d=dx*dx+dz*dz+lanePenalty;
      if(d<faceDist){ faceDist=d; faceTarget=h; }
    }
    if(faceTarget){
      const currentX=board.worldX(this.col);
      const currentZ=board.worldZ(this.row);
      ...
      // No hero remains: keep looking toward the house.
      this.facingY=-Math.PI/2;
    }
