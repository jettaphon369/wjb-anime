...
class Board3D{
  ...
  buildLawn(){
    ...
    // Floating island body. Keep every underside mesh vertical so nothing can cut through the field.
    const soil=new THREE.Mesh(new THREE.BoxGeometry(boardW+1.8,1.20,boardH+1.8),this.makeMat(0x70492d,1));
    soil.position.set(centerX,-.70,0); soil.receiveShadow=true; soil.castShadow=true; this.scene.add(soil);

    // Low-poly rock chunks under the island (replaces the old rotated cylinder that became a wall).
    const rockMat=this.makeMat(0x5f5148,1);
    const rockCols=7, rockRows=4;
    for(let rz=0;rz<rockRows;rz++) for(let rx=0;rx<rockCols;rx++){
      const px=centerX-boardW*.43 + rx*(boardW*.86/(rockCols-1));
      const pz=-boardH*.40 + rz*(boardH*.80/(rockRows-1));
      const edge=Math.min(rx,rockCols-1-rx,rz,rockRows-1-rz);
      const depth=edge===0 ? 1.55 : (edge===1 ? 1.95 : 2.25);
      const radius=edge===0 ? .72 : .88;
      const rock=new THREE.Mesh(new THREE.ConeGeometry(radius,.9+depth,6),rockMat);
      rock.position.set(px,-1.14-depth*.48,pz);
      rock.rotation.y=((rx*0.71+rz*1.17)%1)*Math.PI;
      rock.scale.set(1,1,.82); rock.castShadow=true; rock.receiveShadow=true; this.scene.add(rock);
    }

    const grassTop=new THREE.Mesh(new THREE.BoxGeometry(boardW+1.55,.34,boardH+1.55),this.makeMat(0x5fae38,.96));
  ...
  buildHouse(){
    ...
    g.position.set(this.worldX(-1.82),0,.15); g.rotation.y=.05; this.scene.add(g); this.houseGroup=g;
  }
  resize(){
