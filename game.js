...
const HERO_MODEL_YAW = {
  // Brisky's GLB already faces along the game's forward +X axis.
  brisky:0,
  mio:Math.PI/2,
  gabby:Math.PI/2,
  wind:Math.PI/2,
...
class HeroUnit{
  ...
  update(dt,dtms,board){
    ...
    if(target){
      ...
      const targetX=board.worldX(target.col);
      const targetZ=board.worldZ(target.row);
      const worldAngle=Math.atan2(targetX-currentX,targetZ-currentZ);
      // Default hero visual direction is +X (PI/2), so store only the relative holder turn.
      this.targetFacingY=worldAngle-Math.PI/2;
    }else{
      // No valid target in this lane: keep looking straight ahead toward the enemy side.
      this.targetFacingY=0;
    }
