...
const ModelAssets = {
  ...
  briskyWaiters:[],
  loadBrisky(callback){
    if(this.brisky){ callback(this.cloneBrisky()); return; }
    if(callback) this.briskyWaiters.push(callback);
    if(this.briskyLoading) return;
    this.briskyLoading=true;
    if(typeof THREE==='undefined' || typeof THREE.GLTFLoader==='undefined'){
      console.warn('GLTFLoader unavailable; using Brisky sprite fallback.');
      this.briskyLoading=false;
      this.briskyWaiters.length=0;
      return;
    }
    const loader=new THREE.GLTFLoader();
    loader.load('assets/models/Brisky_SelfMade_V2.glb', gltf=>{
      this.brisky=gltf.scene;
      this.brisky.traverse(o=>{
        if(o.isMesh){
      ...
      const waiters=this.briskyWaiters.splice(0);
      this.briskyLoading=false;
      waiters.forEach(fn=>{ try{ fn(this.cloneBrisky()); }catch(e){ console.error(e); } });
      console.log('Brisky 3D model loaded.');
    }, undefined, err=>{
      console.warn('Could not load Brisky GLB; using sprite fallback.',err);
      this.briskyLoading=false;
      this.briskyWaiters.length=0;
    });
  },
  cloneBrisky(){ return this.brisky ? this.brisky.clone(true) : null; },
  fitBrisky(model){
  ...
  attachBriskyAsync(group){
    group.userData.externalModelPending=true;
    this.loadBrisky(model=>{
      if(!group.parent) return;
      this.fitBrisky(model);
      const old=group.userData.sprite;
      if(old){ group.remove(old); if(old.material) old.material.dispose(); }
      group.add(model);
      group.userData.spriteMode=false;
      group.userData.externalModel=model;
...
const ModelFactory = {
  ...
  buildHero(def){
    if(def.id==='brisky'){
      const g=this.buildHeroSprite(def);
      ModelAssets.attachBriskyAsync(g);
      return g;
    }
...
class HeroUnit{
  ...
  animate(dtms,board){
    this.t+=dtms;
    const bob=Math.sin(this.t/280+this.seed)*0.06;
    const breathe=1+Math.sin(this.t/500+this.seed)*0.03;
...
class Board3D{
  ...
  initThree(){
    ...
    this.scene.background=new THREE.Color(0x1c3a2e);
    this.scene.fog=new THREE.Fog(0x1c3a2e,16,30);
    this.camera=new THREE.PerspectiveCamera(42,1,0.1,100);
    this.camera.position.set(0,9.5,8.5);
    this.camera.lookAt(0,0,0.4);
    this.camBase=this.camera.position.clone();
    const hemi=new THREE.HemisphereLight(0xffffff,0x22331c,0.85);
    this.scene.add(hemi);
  ...
  buildLawn(){
    const grassTex=TextureFactory.grassTile();
    const pathTex=TextureFactory.pathTile();
    grassTex.wrapS=grassTex.wrapT=THREE.RepeatWrapping;
    pathTex.wrapS=pathTex.wrapT=THREE.RepeatWrapping;
    grassTex.repeat.set(this.playableCols,this.rows);
    pathTex.repeat.set(this.cols-this.playableCols,this.rows);

    const grassW=this.playableCols*this.tile, pathW=(this.cols-this.playableCols)*this.tile, boardH=this.rows*this.tile;
    const grassMat=new THREE.MeshStandardMaterial({map:grassTex,roughness:0.95});
    const grassPlane=new THREE.Mesh(new THREE.PlaneGeometry(grassW,boardH),grassMat);
    grassPlane.rotation.x=-Math.PI/2;
    grassPlane.position.set(this.worldX((this.playableCols-1)/2),0,0);
    grassPlane.receiveShadow=true;
    this.scene.add(grassPlane);

    const pathMat=new THREE.MeshStandardMaterial({map:pathTex,roughness:0.95});
    const pathPlane=new THREE.Mesh(new THREE.PlaneGeometry(pathW,boardH),pathMat);
    pathPlane.rotation.x=-Math.PI/2;
    pathPlane.position.set(this.worldX(this.playableCols+(this.cols-this.playableCols-1)/2),0,0);
    pathPlane.receiveShadow=true;
    this.scene.add(pathPlane);

    // thin border trim around the whole board
    const edgeMat=new THREE.MeshStandardMaterial({color:0x2c4a34,roughness:1});
    const edge=new THREE.Mesh(new THREE.BoxGeometry(this.cols*this.tile+0.3,0.18,boardH+0.3),edgeMat);
    edge.position.set(this.worldX((this.cols-1)/2),-0.12,0);
  ...
  buildHouse(){
    const g=new THREE.Group();
    const wall=new THREE.Mesh(new THREE.BoxGeometry(1.6,1.3,1.6), new THREE.MeshStandardMaterial({color:'#f2c88f',roughness:0.7}));
    wall.position.y=0.55; wall.castShadow=true; wall.receiveShadow=true; g.add(wall);
    const roof=new THREE.Mesh(new THREE.ConeGeometry(1.35,1.0,4), new THREE.MeshStandardMaterial({color:'#e0645a',roughness:0.6}));
    roof.rotation.y=Math.PI/4; roof.position.y=1.5; roof.castShadow=true; g.add(roof);
    const door=new THREE.Mesh(new THREE.BoxGeometry(0.45,0.6,0.1), new THREE.MeshStandardMaterial({color:'#8a4b32'}));
    door.position.set(0,0.3,0.83); g.add(door);
    g.position.set(this.worldX(-0.5),0,0);
    this.scene.add(g);
  }
  resize(){
