...
const HERO_MODEL_HEIGHT = {brisky:1.34,mio:1.34,gabby:1.34,wind:1.34,titan:1.34,rian:1.34,kaisen:1.34};
const HERO_MODEL_YAW = {
  // All imported hero models are aligned to the same forward direction (+X).
  // Brisky must use the same quarter-turn as the other heroes.
  brisky:Math.PI/2,
  mio:Math.PI/2,
  gabby:Math.PI/2,
  wind:Math.PI/2,
...
// Sky Alliance races. These are original factions for the game's universe.
const HERO_RACES = {
  brisky:{race:'Beast', icon:'🐺', title:'Sky Vanguard'},
  mio:{race:'Demon', icon:'😈', title:'Arcane Wanderer'},
  gabby:{race:'Beast', icon:'🦆', title:'Island Engineer'},
  wind:{race:'Divine', icon:'✨', title:'Gale Sentinel'},
  titan:{race:'Human', icon:'🛡️', title:'Iron Guardian'},
  rian:{race:'Human', icon:'⚔️', title:'Rift Hunter'},
  kaisen:{race:'Demon', icon:'🌑', title:'Twin-Blade Exile'}
};

const SKY_ALLIANCE_LORE = {
  chapter:'CHAPTER 1',
  stage:'THE LAST SKY ISLAND',
  objective:'Protect the Sky Core from the Void Legion'
};
...
function getVisibleModelBounds(root){
  ...
  root.traverse(o=>{
    ...
    tempBox.copy(o.geometry.boundingBox).applyMatrix4(o.matrixWorld);
    if(!found){ out.copy(tempBox); found=true; }
    else out.union(tempBox);
  });
  return found ? out : new THREE.Box3().setFromObject(root);
}
...
const ModelFactory = {
  ...
  buildHeroPlaceholder3D(def){
    const g=new THREE.Group();
    const primary=this.mat(def.colors[0],{roughness:.72,metalness:.02});
    const secondary=this.mat(def.colors[1],{roughness:.78,metalness:.01});
    const cream=this.mat(0xf2d9b3,{roughness:.88,metalness:0});
    const dark=this.mat(0x211820,{roughness:1,metalness:0});

    const shadow=new THREE.Mesh(new THREE.CircleGeometry(.26,24),new THREE.MeshBasicMaterial({color:0x000000,transparent:true,opacity:.16,depthWrite:false}));
    shadow.rotation.x=-Math.PI/2; shadow.position.y=.012; g.add(shadow);

    const body=new THREE.Mesh(new THREE.SphereGeometry(.34,18,14),primary);
    body.scale.set(.88,1.08,.76); body.position.y=.49; g.add(body);
    const head=new THREE.Mesh(new THREE.SphereGeometry(.29,18,14),primary.clone());
    head.position.set(0,.99,.01); g.add(head);
    const muzzle=new THREE.Mesh(new THREE.SphereGeometry(.15,14,10),cream);
    muzzle.scale.set(1.15,.7,.72); muzzle.position.set(0,.91,.24); g.add(muzzle);
    const nose=new THREE.Mesh(new THREE.SphereGeometry(.045,8,8),dark);
    nose.position.set(0,.94,.36); g.add(nose);

    [-1,1].forEach(side=>{
      const eye=new THREE.Mesh(new THREE.SphereGeometry(.037,8,8),dark.clone());
      eye.position.set(.10*side,1.06,.275); g.add(eye);
      const arm=new THREE.Mesh(new THREE.CylinderGeometry(.052,.064,.36,8),secondary.clone());
      arm.position.set(.32*side,.55,0); arm.rotation.z=-.22*side; g.add(arm);
      const leg=new THREE.Mesh(new THREE.CylinderGeometry(.06,.07,.27,8),secondary.clone());
      leg.position.set(.13*side,.18,0); g.add(leg);
      const foot=new THREE.Mesh(new THREE.SphereGeometry(.085,10,8),primary.clone());
      foot.scale.set(1.2,.56,1.35); foot.position.set(.13*side,.04,.08); g.add(foot);
      let ear;
      if(def.type==='cat'||def.type==='elf'){
        ear=new THREE.Mesh(new THREE.ConeGeometry(.105,.28,8),secondary.clone());
        ear.position.set(.18*side,1.27,0); ear.rotation.z=.42*side;
      }else{
        ear=new THREE.Mesh(new THREE.SphereGeometry(.10,10,8),secondary.clone());
        ear.position.set(.20*side,1.23,0);
      }
      g.add(ear);
    });

    const weaponMat=this.mat(0x59616c,{roughness:.42,metalness:.34});
    const weapon=new THREE.Mesh(new THREE.CylinderGeometry(.024,.034,.60,8),weaponMat);
    weapon.position.set(.40,.56,.05); weapon.rotation.z=-.42; g.add(weapon);
    g.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;}});
    g.userData.placeholder3D=true;
    return g;
  },
  ...
  preloadHeroModels(){
    ...
    Object.values(HERO_MODELS).forEach(path=>{
      if(this._modelCache[path]) return;
      this._gltfLoader.load(path,gltf=>{this._modelCache[path]=gltf.scene;},undefined,err=>console.warn('Preload failed',path,err));
    });
  },
  ...
  buildHeroModel(def){
    ...
    // Always show a real 3D body while the GLB is downloading.
    const temp=this.buildHeroPlaceholder3D(def);
    temp.scale.setScalar(.92); holder.add(temp);
    holder.userData.fallback3D=temp;
    ...
    const install=(source)=>{
      ...
      model.rotation.y=(HERO_MODEL_YAW[def.id]!==undefined)?HERO_MODEL_YAW[def.id]:Math.PI/2;

      // Hide prototype stands BEFORE measuring the model.
      // Otherwise the invisible stand is counted in the bounding box and the hero floats.
      model.traverse(o=>{
        const nodeName=(o.name||'').toLowerCase();
        if(o.isMesh && /(^|[_\- ])(displaybase|base|platform|pedestal|stand)([_\- ]|$)/i.test(nodeName)){
          o.visible=false;
        }
      });

      // Normalize using visible character geometry only, then place the lowest visible point on y=0.
      model.updateMatrixWorld(true);
      let box=getVisibleModelBounds(model);
      const rawHeight=Math.max(0.001,box.max.y-box.min.y);
      const targetHeight=HERO_MODEL_HEIGHT[def.id]||1.34;
      const uniformScale=targetHeight/rawHeight;
      model.scale.setScalar(uniformScale);
      model.updateMatrixWorld(true);

      // Recalculate from visible character meshes only after scaling.
      box=getVisibleModelBounds(model);
      ...
      // Final safety pass: force the lowest visible point to y=0 exactly.
      const finalBox=getVisibleModelBounds(model);
    ...
    if(this._modelCache[path]){
      ...
      this._gltfLoader.load(path,(gltf)=>{
        ...
        holder.userData.loading3D=false;
        holder.userData.modelLoadFailed=true;
      });
    }
    return holder;
  },
  buildHero(def){
    ...
    return this.buildHeroPlaceholder3D(def);
    const g=new THREE.Group();
  ...
  buildMuzzleFlash(color=0xffe07a){
    ...
    for(let i=0;i<3;i++){
      ...
      ray.rotation.x=i*Math.PI/1.5;
      ray.position.x=.10;
      g.add(ray);
    }
    g.userData.fxLife=.16;
    g.userData.fxAge=0;
    return g;
  },
  ...
  buildEnemy(def,typeId){
    ...
    const bodyMat=new THREE.MeshStandardMaterial({color:def.color,roughness:0.62,transparent,opacity:def.alpha||1});
    let body;

    if(typeId==='slime'){
      body=new THREE.Mesh(new THREE.SphereGeometry(.36*s,18,12),bodyMat);
      body.scale.set(1.12,.78,1.05); body.position.y=.30*s; g.add(body);
      const cap=new THREE.Mesh(new THREE.SphereGeometry(.22*s,14,10),bodyMat.clone());
      cap.scale.set(1,.55,1); cap.position.y=.57*s; g.add(cap);
    }else if(typeId==='wolf'){
      body=new THREE.Mesh(new THREE.CapsuleGeometry(.22*s,.44*s,5,10),bodyMat);
      body.rotation.z=Math.PI/2; body.position.y=.34*s; g.add(body);
      const head=new THREE.Mesh(new THREE.SphereGeometry(.20*s,14,10),bodyMat.clone());
      head.position.set(-.31*s,.43*s,0); g.add(head);
      for(const z of [-.11,.11]){
        const leg=new THREE.Mesh(new THREE.CylinderGeometry(.04*s,.05*s,.28*s,7),bodyMat.clone());
        leg.position.set(.05*s,.14*s,z*s); g.add(leg);
      }
    }else if(typeId==='skeleton'){
      body=new THREE.Group();
      const boneMat=new THREE.MeshStandardMaterial({color:0xe5e0cc,roughness:.85});
      const skull=new THREE.Mesh(new THREE.SphereGeometry(.20*s,14,10),boneMat); skull.position.y=.66*s; body.add(skull);
      const spine=new THREE.Mesh(new THREE.CylinderGeometry(.035*s,.045*s,.42*s,7),boneMat); spine.position.y=.35*s; body.add(spine);
      [-1,1].forEach(side=>{
        const arm=new THREE.Mesh(new THREE.CylinderGeometry(.025*s,.032*s,.34*s,7),boneMat);
        arm.position.set(.19*s*side,.42*s,0); arm.rotation.z=.55*side; body.add(arm);
        const leg=new THREE.Mesh(new THREE.CylinderGeometry(.028*s,.036*s,.30*s,7),boneMat);
        leg.position.set(.09*s*side,.10*s,0); body.add(leg);
      });
      g.add(body);
    }else if(typeId==='darkmage'){
      body=new THREE.Mesh(new THREE.ConeGeometry(.28*s,.64*s,12),bodyMat);
      body.position.y=.32*s; g.add(body);
      const hood=new THREE.Mesh(new THREE.SphereGeometry(.19*s,14,10),new THREE.MeshStandardMaterial({color:0x261534,roughness:.82}));
      hood.position.y=.69*s; g.add(hood);
    }else if(typeId==='orc'||typeId==='ogre'||typeId==='boss'){
      body=new THREE.Mesh(new THREE.CapsuleGeometry(.28*s,.48*s,6,12),bodyMat);
      body.position.y=.46*s; g.add(body);
      const chest=new THREE.Mesh(new THREE.BoxGeometry(.52*s,.30*s,.34*s),bodyMat.clone());
      chest.position.y=.52*s; g.add(chest);
    }else{
      body=new THREE.Mesh(new THREE.CapsuleGeometry(.22*s,.36*s,5,10),bodyMat);
      body.position.y=.38*s; g.add(body);
    }

    const eyeMat=new THREE.MeshStandardMaterial({color:'#ff3b3b',emissive:'#ff3b3b',emissiveIntensity:0.85});
    [-0.1,0.1].forEach(ex=>{
      const eye=new THREE.Mesh(new THREE.SphereGeometry(0.045*s,8,8),eyeMat);
      eye.position.set(ex*s,0.53*s,0.25*s); g.add(eye);
    });
...
class HeroUnit{
  constructor(defId,row,col){
    ...
    this.attackAnimT=0;
    this.hitAnimT=0;
    // Holder rotation is relative to the default direction toward the enemy side (+X).
    this.facingY=0;
  ...
  buildMesh(board){
    ...
    this.mesh.position.set(board.worldX(this.col),board.heroGroundY,board.worldZ(this.row));
    // Child GLBs use HERO_MODEL_YAW; the holder starts facing the enemy side.
  ...
  takeDamage(dmg){
    this.hp-=dmg; this.state='hit'; this.stateTimer=180; this.hitAnimT=1;
    if(this.mesh) Game.fx.burst(this.mesh.position.x,0.7,this.mesh.position.z,'#ff5a5a',8);
  ...
  update(dt,dtms,board){
    ...
    // Only face a living enemy in this hero's own lane and in front of the hero.
    // Never turn toward enemies in another row.
    // Heroes are lane defenders: always keep their body facing straight toward +X.
    // Target selection remains limited to the same row and enemies in front.
    this.targetFacingY=0;
  ...
  attack(target,board){
    this.state='attack'; this.stateTimer=260; this.attackAnimT=1; Audio2.play('attack');
    const sx=board.worldX(this.col)+.38;
    const sz=board.worldZ(this.row);
    if(!this.def.melee) board.spawnMuzzleFx(sx,.78,sz,ModelFactory.projColor(this.def.proj||'arrow'));
  ...
  animate(dtms,board){
    this.t+=dtms;
    const dt=dtms/1000;
    this.attackAnimT=Math.max(0,this.attackAnimT-dt*4.8);
    this.hitAnimT=Math.max(0,this.hitAnimT-dt*6.2);

    const idle=Math.sin(this.t/520+this.idlePhase);
    const breathe=1+idle*0.010;
    const attackProgress=1-this.attackAnimT;
    const attackPulse=this.attackAnimT>0 ? Math.sin(Util.clamp(attackProgress,0,1)*Math.PI) : 0;
    const anticipation=this.attackAnimT>0 ? Math.sin(Util.clamp(attackProgress*1.7,0,1)*Math.PI)*0.055 : 0;
    const hitPulse=this.hitAnimT;

    const meleeLunge=this.def.melee ? attackPulse*0.18-anticipation : -attackPulse*0.055;
    const hitKnock=-hitPulse*0.09;
    const idleSway=Math.sin(this.t/900+this.idlePhase)*0.012;

    this.mesh.position.set(
      board.worldX(this.col)+meleeLunge+hitKnock,
      board.heroGroundY,
      board.worldZ(this.row)
    );

    const squash=attackPulse*0.045;
    this.mesh.scale.set(
      breathe*(1+squash),
      breathe*(1-squash*0.65),
      breathe*(1+squash)
    );

    let turnDelta=this.targetFacingY-this.facingY;
    turnDelta=Math.atan2(Math.sin(turnDelta),Math.cos(turnDelta));
    this.facingY+=turnDelta*0.20;
    this.mesh.rotation.y=this.facingY+idleSway;
    this.mesh.rotation.z=(attackPulse*0.05-hitPulse*0.08);

    const modelRoot=this.mesh.userData.modelRoot;
    if(modelRoot){
      // Animate the imported GLB without lifting its holder off the tile.
      modelRoot.position.y=Math.max(0, Math.abs(idle)*0.014 + attackPulse*0.018);
      modelRoot.rotation.x=-attackPulse*0.08;
    }

    if(this.mesh.userData.rightArm) this.mesh.userData.rightArm.rotation.x=-attackPulse*1.20;
    if(this.mesh.userData.leftArm) this.mesh.userData.leftArm.rotation.x=-attackPulse*0.45;

    if(this.mesh.userData.spriteMode && this.mesh.userData.sprite){
      const s=this.mesh.userData.spriteBaseScale;
      const punch=1+attackPulse*0.12;
      this.mesh.userData.sprite.scale.set(s.x*punch,s.y*(1-attackPulse*0.05),1);
    }

    setEmissive(this.mesh, hitPulse>0?0xff3b3b:0x000000, hitPulse*0.85);
  }
  animateDeath(dtms){
    if(!this.mesh) return;
...
class EnemyUnit{
  constructor(typeId,row,col,mult){
    ...
    // Enemy models are visually built facing +Z.
    // Start by turning toward the heroes/house on the left side of the field.
    this.facingY=-Math.PI/2;
    this.mesh=null;
  }
  buildMesh(board){
  ...
  dispose(board){ if(this.mesh){ board.unitGroup.remove(this.mesh); disposeGroup(this.mesh); this.mesh=null; } }
  takeDamage(dmg){
    this.hp-=dmg; if(!this.dead){ this.state='hit'; this.stateTimer=140; this.hitPush=Math.min(.16,this.hitPush+.075); }
    if(this.mesh) Game.fx.burst(this.mesh.position.x,0.5,this.mesh.position.z,this.def.color,7);
    if(this.hp<=0 && !this.dead){
      ...
      if(this.def.boss){
        Game.screenShake(0.26,520);
        const prevSpeed=Game.speed;
        Game.speed=.35;
        setTimeout(()=>{ if(Game.running) Game.speed=prevSpeed; },420);
      }
  ...
  update(dt,dtms,board){
    ...
    // Enemies focus only on heroes directly ahead in their own lane.
    // This prevents them from turning sideways toward another row.
    let faceTarget=null;
    let nearestForward=Infinity;
    for(const h of board.heroes){
      if(h.dead || h.row!==this.row) continue;
      const dx=this.col-h.col;
      if(dx>=-0.1 && dx<nearestForward){
        nearestForward=dx;
        faceTarget=h;
      }
    }
    if(faceTarget){
      const currentX=board.worldX(this.col);
      const currentZ=board.worldZ(this.row);
      const targetX=board.worldX(faceTarget.col);
      const targetZ=board.worldZ(faceTarget.row);
      this.facingY=Math.atan2(targetX-currentX,targetZ-currentZ);
    }else{
      // No hero in this lane: continue facing straight toward the house.
      this.facingY=-Math.PI/2;
    }
  ...
  animate(dtms,board){
    ...
    this.t+=dtms;
    const dt=dtms/1000;
    this.hitPush=Math.max(0,this.hitPush-dt*.55);
    const walkPhase=this.t/165+this.walkPhase;
    const walkBob=this.state==='walk'?Math.abs(Math.sin(walkPhase))*0.028:0;
    const walkTilt=this.state==='walk'?Math.sin(walkPhase)*0.055:0;
    const attackKick=this.state==='attack'?Math.sin(Util.clamp(1-this.stateTimer/250,0,1)*Math.PI)*0.13:0;
    this.mesh.position.set(board.worldX(this.col)-attackKick+this.hitPush,walkBob,board.worldZ(this.row));
    this.mesh.rotation.z=walkTilt-(this.state==='hit'?0.06:0);
    const hitScale=this.state==='hit'?0.94:1;
    this.mesh.scale.set(hitScale,1+(1-hitScale)*.7,hitScale);

    // Smooth shortest-path rotation so enemies visibly turn toward our units.
    let delta=this.facingY-this.mesh.rotation.y;
    delta=Math.atan2(Math.sin(delta),Math.cos(delta));
    this.mesh.rotation.y+=delta*0.18;
  ...
  animateDeath(dtms){
    if(!this.mesh) return;
    const t=Util.clamp(this.fadeT/520,0,1);
    this.mesh.position.y=Math.max(-.12,this.mesh.position.y-0.0011*dtms);
    this.mesh.rotation.x=t*1.55;
    this.mesh.rotation.z+=0.0024*dtms*(this.id%2?1:-1);
    const s=Math.max(.02,1-t*.82);
...
class Projectile3D{
  constructor(board,shooter,target,dmg,type,opts){
    ...
    const glow=new THREE.PointLight(ModelFactory.projColor(type),.55,.9);
    glow.position.set(0,0,0);
    this.mesh.add(glow);
    this.mesh.userData.glow=glow;
  ...
  update(dt,board){
    ...
    if(d<=step){
      ...
      Game.showDamageNumber(this.target,this.dmg,false);
      board.spawnImpactFx(tx,tz,ModelFactory.projColor(this.type));
      Game.fx.burst(tx,ty,tz,ModelFactory.projColor(this.type),10);
      this.dead=true;
    } else {
...
class Board3D{
  constructor(canvas){
    ...
    this.tile=1.6;
    // Grass tile center is y=.05 and tile height is .13, so its top is .115.
    this.heroGroundY=0.115;
    this.raycaster=new THREE.Raycaster();
    this.initThree();
    ModelFactory.preloadHeroModels();
    this.buildLawn();
    this.buildHouse();
    this.resize();
  }
  ...
  initThree(){
    ...
    // Softer mobile-friendly color grading: prevents neon/white clipped grass.
    ...
    this.scene.fog=new THREE.Fog(0xa8d9ef,24,48);
    this.camera=new THREE.PerspectiveCamera(42,1,0.1,120);
    this.camera.position.set(-1.2,12.8,15.8);
    this.camera.lookAt(0,0,0);

    // 360-degree camera controls: drag to rotate, pinch/wheel to zoom.
    ...
    const ambient=new THREE.AmbientLight(0xf7fbff,0.11);
    this.scene.add(ambient);
    const hemi=new THREE.HemisphereLight(0xbfdcf2,0x233526,0.34);
    this.scene.add(hemi);
    const dir=new THREE.DirectionalLight(0xffe2b8,0.72);
    dir.position.set(-8,13,7);
    dir.castShadow=true;
    ...
    this.scene.add(dir);
    const fill=new THREE.DirectionalLight(0xb7d8ff,0.12);
    fill.position.set(7,6,-5);
    this.scene.add(fill);
    const rim=new THREE.DirectionalLight(0xffd7ad,0.06);
    ...
    this.combatFxGroup=new THREE.Group(); this.scene.add(this.combatFxGroup);
  }
  makeSkyTexture(){
    const c=document.createElement('canvas'); c.width=1024; c.height=512;
    const x=c.getContext('2d');
    const g=x.createLinearGradient(0,0,0,512);
    g.addColorStop(0,'#4a9ed8');
    g.addColorStop(.42,'#83c5e7');
    g.addColorStop(.72,'#c4e5ef');
    g.addColorStop(1,'#f0eee4');
    x.fillStyle=g; x.fillRect(0,0,1024,512);

    // Warm sun glow.
    const sun=x.createRadialGradient(780,110,10,780,110,150);
    sun.addColorStop(0,'rgba(255,244,201,.95)');
    sun.addColorStop(.25,'rgba(255,222,151,.45)');
    sun.addColorStop(1,'rgba(255,220,150,0)');
    x.fillStyle=sun; x.fillRect(620,0,320,300);

    // Softer, less cartoon-flat clouds.
    x.globalAlpha=.52;
    for(let i=0;i<18;i++){
      const cx=Math.random()*1024, cy=60+Math.random()*280, r=18+Math.random()*42;
      x.fillStyle='rgba(255,255,255,.92)';
      x.beginPath();
      x.ellipse(cx,cy,r*1.5,r*.72,0,0,Math.PI*2);
      x.ellipse(cx+r*.85,cy+r*.03,r*.92,r*.62,0,0,Math.PI*2);
      x.ellipse(cx-r*.85,cy+r*.10,r*.75,r*.52,0,0,Math.PI*2);
      x.fill();
    }
    x.globalAlpha=1;
  ...
  makeGroundTexture(kind='grass'){
    const c=document.createElement('canvas'); c.width=256; c.height=256;
    const x=c.getContext('2d');
    const isGrass=kind==='grass';
    x.fillStyle=isGrass?'#6f9d43':'#70503a';
    x.fillRect(0,0,256,256);

    // Multi-scale natural color variation.
    for(let i=0;i<2200;i++){
      const px=Math.random()*256, py=Math.random()*256;
      const a=.025+Math.random()*.07;
      x.fillStyle=isGrass
        ? (Math.random()>.5?`rgba(190,220,110,${a})`:`rgba(30,80,35,${a})`)
        : (Math.random()>.5?`rgba(180,125,75,${a})`:`rgba(45,28,18,${a})`);
      const s=.35+Math.random()*1.6;
    ...
    if(isGrass){
      ...
      for(let i=0;i<420;i++){
        ...
        x.strokeStyle=Math.random()>.5?'rgba(42,105,42,.28)':'rgba(205,230,120,.22)';
        x.beginPath(); x.moveTo(px,py); x.lineTo(px+(Math.random()-.5)*1.5,py-len); x.stroke();
      }
    }

    const t=new THREE.CanvasTexture(c);
    t.wrapS=t.wrapT=THREE.RepeatWrapping;
    t.repeat.set(isGrass?3.2:2.2,isGrass?3.2:2.2);
    t.encoding=THREE.sRGBEncoding;
    t.anisotropy=Math.min(8,this.renderer.capabilities.getMaxAnisotropy());
    return t;
  }

  makeGrassMat(color=0xffffff,rough=.94){
    if(!this._grassTex) this._grassTex=this.makeGroundTexture('grass');
    const m=new THREE.MeshStandardMaterial({
      color,
      map:this._grassTex,
      bumpMap:this._grassTex,
      bumpScale:.035,
      roughness:rough,
      metalness:0
    });
  ...
  makeSoilMat(color=0xffffff){
    if(!this._soilTex) this._soilTex=this.makeGroundTexture('soil');
    return new THREE.MeshStandardMaterial({
      color,
      map:this._soilTex,
      bumpMap:this._soilTex,
      bumpScale:.05,
      roughness:1,
      metalness:0
    });
  ...
  makeMat(color,rough=.9){ return new THREE.MeshStandardMaterial({color,roughness:rough,metalness:0}); }
  addRock(x,z,s=.35){
    const rockMat=this.makeMat(0x7f8582,.88);
    rockMat.metalness=.02;
  ...
  buildLawn(){
    ...
    // Large circular floating island. The rectangular battle grid sits safely on top.
    const islandRadius=11.6;
    const soil=new THREE.Mesh(
      new THREE.CylinderGeometry(islandRadius,islandRadius*.87,1.35,48,1,false),
      this.makeSoilMat(0x8a684f)
    );
    soil.position.set(centerX,-.78,0);
    soil.receiveShadow=true; soil.castShadow=true; this.scene.add(soil);

    // Tapered rocky underside creates a true floating-island silhouette.
    const lowerRock=new THREE.Mesh(
      new THREE.ConeGeometry(islandRadius*.84,5.0,36,1,false),
      this.makeSoilMat(0x66564e)
    );
    lowerRock.position.set(centerX,-3.55,0);
    lowerRock.rotation.y=Math.PI/36;
    lowerRock.castShadow=true; lowerRock.receiveShadow=true; this.scene.add(lowerRock);

    // Faceted rock band beneath the soil.
    const rockMat=this.makeMat(0x67574d,1);
    for(let i=0;i<30;i++){
      const a=(i/30)*Math.PI*2;
      const r=islandRadius*.72 + Math.sin(i*2.17)*.55;
      const rock=new THREE.Mesh(
        new THREE.ConeGeometry(.72+((i%4)*.08),2.1+(i%3)*.35,6),
        rockMat
      );
      rock.position.set(centerX+Math.cos(a)*r,-1.95-Math.sin(i*.91)*.22,Math.sin(a)*r);
      rock.rotation.y=-a;
      rock.rotation.z=(Math.sin(i*.73))*0.12;
      rock.scale.z=.78;
      rock.castShadow=true; rock.receiveShadow=true; this.scene.add(rock);
    }

    // Thick circular grass cap.
    const grassTop=new THREE.Mesh(
      new THREE.CylinderGeometry(islandRadius,islandRadius-.12,.38,64),
      this.makeGrassMat(0x89b95b,.96)
    );
    grassTop.position.set(centerX,-.14,0);
    grassTop.receiveShadow=true; grassTop.castShadow=true; this.scene.add(grassTop);

    // Soft outer grass ring adds a rounded natural edge.
    const grassRim=new THREE.Mesh(
      new THREE.TorusGeometry(islandRadius-.15,.22,8,64),
      this.makeGrassMat(0x76ad4c,.95)
    );
    grassRim.rotation.x=Math.PI/2;
    grassRim.position.set(centerX,.02,0);
    grassRim.castShadow=true; grassRim.receiveShadow=true; this.scene.add(grassRim);
    ...
    for(let row=0; row<this.rows; row++) for(let col=0; col<this.cols; col++){
      ...
      const base=isPath ? (0x719b45 + ((row+col)%2?0x020300:0)) : (0x69a640 + ((row+col)%2?0x020400:0));
      const tile=new THREE.Mesh(tileGeo,this.makeGrassMat(base,.95));
      tile.position.set(this.worldX(col),.05+Math.sin((row+1)*(col+2))*.006,this.worldZ(row));
      tile.rotation.y=((row*13+col*7)%5-2)*0.0015;
      tile.receiveShadow=true; tile.castShadow=false; this.scene.add(tile);
      const seam=new THREE.LineSegments(new THREE.EdgesGeometry(tileGeo),new THREE.LineBasicMaterial({color:0x3f762c,transparent:true,opacity:.20})); seam.position.copy(tile.position); this.scene.add(seam);
    }
    ...
    // Decorations around the circular perimeter.
    for(let i=0;i<18;i++){
      const a=(i/18)*Math.PI*2 + .12;
      const rr=islandRadius-.72;
      const x=centerX+Math.cos(a)*rr, z=Math.sin(a)*rr;
      if(i%5===0) this.addRock(x,z,.30+(i%3)*.05);
      else this.addFlower(x,z,Util.choice([0xffffff,0xffd34f,0xff8ca0,0xaedcff]));
    }
  ...
  buildHouse(){
    ...
    // House stays on the large circular island, but is moved clear of the battle grid.
    g.scale.set(1.18,1.18,1.18);
    g.position.set(this.worldX(-2.18),0,.25);
    g.rotation.y=.08;
    this.scene.add(g); this.houseGroup=g;
  }
  resize(){
  ...
  spawnProjectile(hero,target){
    if(hero.def.proj==='none'){
      const isCrit=!!hero.def.crit && Math.random()<0.22;
      const damage=hero.dmg*(isCrit?2:1);
      target.takeDamage(damage);
      Game.showDamageNumber(target,damage,isCrit);
      this.spawnImpactFx(this.worldX(target.col),this.worldZ(target.row),isCrit?0xffd166:0xffffff);
      Game.fx.burst(this.worldX(target.col),0.72,this.worldZ(target.row),isCrit?0xffd166:0xffffff,isCrit?14:8,{big:isCrit});
      if(isCrit) Game.screenShake(0.08,110);
      return;
    }
  ...
  spawnImpactFx(x,z,color=0xffffff){
    const fx=ModelFactory.buildImpactRing(color);
    fx.position.set(x,.125,z);
    this.combatFxGroup.add(fx);
  }
  spawnMuzzleFx(x,y,z,color=0xffe07a){
    const fx=ModelFactory.buildMuzzleFlash(color);
    fx.position.set(x,y,z);
    fx.rotation.y=Math.PI/2;
    this.combatFxGroup.add(fx);
  }
  updateCombatFx(dt){
    for(let i=this.combatFxGroup.children.length-1;i>=0;i--){
      const fx=this.combatFxGroup.children[i];
      fx.userData.fxAge=(fx.userData.fxAge||0)+dt;
      const life=fx.userData.fxLife||.25;
      const t=Math.min(1,fx.userData.fxAge/life);
      fx.scale.setScalar(.65+t*1.9);
      fx.traverse(o=>{if(o.material && 'opacity' in o.material) o.material.opacity=1-t;});
      if(t>=1){
        this.combatFxGroup.remove(fx);
        disposeGroup(fx);
      }
    }
  }
  damageHouse(v){
    ...
    Game.flashBattleScreen('rgba(255,70,70,.18)',140);
  ...
  render(){
    ...
    if(Game.shakeT>0){
      ...
      this.camera.position.x+=(Math.random()-0.5)*Game.shakeMag;
      this.camera.position.y+=(Math.random()-0.5)*Game.shakeMag*.4;
      this.camera.position.z+=(Math.random()-0.5)*Game.shakeMag;
...
const Game = {
  ...
  coreCharge:0, coreReady:false, waveBannerTimer:0,
  ...
  start(levelNum){
    ...
    this.sessionKills=0;
    this.coreCharge=0; this.coreReady=false; this.waveBannerTimer=0;
    this.ensureSkyAllianceUI();
    this.updateCoreButton();
    document.getElementById('endgameOverlay').classList.remove('active');
    document.getElementById('pauseOverlay').classList.remove('active');
    this.loadWave(0);
    this.buildCardTray();
    document.getElementById('levelLabel').textContent=
      SKY_ALLIANCE_LORE.chapter+' · '+SKY_ALLIANCE_LORE.stage+
      ' · LV '+levelNum+(this.levelConf.isBoss?' 👑':'');
    this.updateHud();
  ...
  loadWave(i){
    ...
    this.showWaveBanner(i+1, this.levelConf.waves.length);
  },
  buildCardTray(){
    ...
    for(const id of Storage.data.unlockedHeroes){
      ...
      drawCardIcon(cv,def);
      const race=HERO_RACES[id]||{icon:'✦',race:'Unknown'};
      const badge=document.createElement('div');
      badge.className='race-badge';
      badge.textContent=race.icon;
      badge.title=race.race+' · '+race.title;
  ...
  flashBattleScreen(color='rgba(255,255,255,.14)',duration=120){
    const battle=document.getElementById('screen-battle');
    if(!battle) return;
    let fx=document.getElementById('battleFlashFx');
    if(!fx){
    ...
    fx.style.opacity='1';
    clearTimeout(this._battleFlashTimer);
    this._battleFlashTimer=setTimeout(()=>{fx.style.opacity='0';},duration);
  },

  showDamageNumber(unit,amount,isCrit=false){
    if(!unit || !unit.mesh || !this.board) return;
    const pos=unit.mesh.position.clone();
    pos.y += unit instanceof EnemyUnit ? (unit.def.boss?1.45:.95) : 1.45;
    this.floatTextWorld(pos,(isCrit?'CRIT ':'')+'-'+Math.round(amount));
  },
  ...
  ensureSkyAllianceUI(){
    ...
    if(!document.getElementById('skyAllianceStyle')){
      const style=document.createElement('style');
      style.id='skyAllianceStyle';
      style.textContent=`
        #stageObjective{position:absolute;left:50%;top:78px;transform:translateX(-50%);
          z-index:15;padding:5px 12px;border-radius:999px;background:rgba(13,25,40,.68);
          border:1px solid rgba(170,225,255,.32);color:#e9f7ff;font:700 10px/1.2 system-ui;
          letter-spacing:.5px;text-shadow:0 1px 2px #000;pointer-events:none;white-space:nowrap}
        #coreSkillBtn{position:absolute;right:14px;bottom:58px;z-index:20;width:74px;height:74px;
          border-radius:50%;border:3px solid #91c9e8;background:radial-gradient(circle at 38% 32%,#8beaff,#234d86 58%,#12192c 100%);
          box-shadow:0 0 0 4px rgba(9,20,38,.75),0 6px 16px rgba(0,0,0,.4),inset 0 0 18px rgba(255,255,255,.25);
          color:white;font:900 11px/1.05 system-ui;text-align:center;padding:0;overflow:hidden;touch-action:manipulation}
        #coreSkillBtn.ready{animation:corePulse .85s infinite alternate;border-color:#fff18a;box-shadow:0 0 18px #79e8ff,0 0 34px #ffe979}
        #coreSkillBtn:disabled{filter:saturate(.55);opacity:.82}
        #coreFill{position:absolute;left:0;right:0;bottom:0;background:rgba(92,225,255,.55);height:0%;transition:height .2s}
        #coreSkillText{position:relative;z-index:2;text-shadow:0 2px 3px #000}
        #waveBanner{position:absolute;left:50%;top:42%;transform:translate(-50%,-50%) scale(.85);
          z-index:30;opacity:0;pointer-events:none;color:#fff;text-align:center;
          font:900 28px/1 system-ui;letter-spacing:2px;text-shadow:0 3px 0 #173451,0 0 18px #6ddcff;
          transition:.25s ease}
        #waveBanner.active{opacity:1;transform:translate(-50%,-50%) scale(1)}
        #waveBanner small{display:block;margin-top:8px;font-size:11px;letter-spacing:1px;color:#d8f5ff}
        .tray-card .race-badge{position:absolute;left:3px;top:3px;width:18px;height:18px;border-radius:50%;
          display:grid;place-items:center;background:rgba(10,15,28,.85);border:1px solid rgba(255,255,255,.45);
          font-size:11px;z-index:3}
        @keyframes corePulse{from{transform:scale(1)}to{transform:scale(1.07)}}
        @media(max-width:600px){#stageObjective{top:72px;font-size:8px}#coreSkillBtn{width:64px;height:64px;right:8px;bottom:55px}}
      `;
      document.head.appendChild(style);
    }

    if(!document.getElementById('stageObjective')){
      const objective=document.createElement('div');
      objective.id='stageObjective';
      objective.textContent='✦ '+SKY_ALLIANCE_LORE.objective;
      battle.appendChild(objective);
    }

    if(!document.getElementById('coreSkillBtn')){
      const btn=document.createElement('button');
      btn.id='coreSkillBtn';
      btn.type='button';
      btn.innerHTML='<div id="coreFill"></div><div id="coreSkillText">SKY CORE<br><span id="corePct">0%</span></div>';
      btn.onclick=()=>Game.activateSkyCore();
      battle.appendChild(btn);
    }

    if(!document.getElementById('waveBanner')){
  ...
  updateCoreButton(){
    const btn=document.getElementById('coreSkillBtn');
    const fill=document.getElementById('coreFill');
    const pct=document.getElementById('corePct');
    if(!btn||!fill||!pct) return;
    const value=Math.floor(this.coreCharge);
    fill.style.height=value+'%';
    pct.textContent=this.coreReady?'READY':value+'%';
    btn.classList.toggle('ready',this.coreReady);
    btn.disabled=!this.coreReady;
  },

  activateSkyCore(){
    if(!this.coreReady||!this.board||!this.running||this.paused) return;
    Audio2.play('collect');
    this.flashBattleScreen('rgba(110,225,255,.20)',220);
    this.coreCharge=0; this.coreReady=false; this.updateCoreButton();

    const board=this.board;
    const centerX=board.worldX(Math.max(2,board.playableCols*.55));
    const centerZ=board.worldZ((board.rows-1)/2);
    this.fx.burst(centerX,.8,centerZ,0x73e6ff,60,{big:true});
    this.fx.burst(centerX,.55,centerZ,0xffe776,38,{big:true});
    this.screenShake(.20,420);

    for(const enemy of [...board.enemies]){
      if(!enemy.dead) enemy.takeDamage(Math.max(90,enemy.maxHp*.42));
    }
    for(const hero of board.heroes){
      if(hero.dead) continue;
      hero.hp=Math.min(hero.maxHp,hero.hp+hero.maxHp*.30);
    }
    board.houseHp=Math.min(board.houseMaxHp,board.houseHp+15);
    this.floatTextWorld(new THREE.Vector3(centerX,.8,centerZ),'SKY CORE PULSE!');
    this.updateHud();
  },

  showWaveBanner(current,total){
    const banner=document.getElementById('waveBanner');
    if(!banner) return;
    banner.innerHTML='WAVE '+current+'<small>VOID LEGION · '+current+' / '+total+'</small>';
    banner.classList.add('active');
    clearTimeout(this._waveBannerTimeout);
    this._waveBannerTimeout=setTimeout(()=>banner.classList.remove('active'),1250);
  },
  ...
  floatText(x,y,text){
    ...
    const isDamage=String(text).includes('-');
    if(isDamage){
      el.style.color=String(text).includes('CRIT')?'#ffd166':'#ffffff';
      el.style.fontSize=String(text).includes('CRIT')?'17px':'14px';
      el.style.textShadow='0 2px 3px #000,0 0 8px rgba(255,80,80,.7)';
    }
    ...
    let t=0; const anim=()=>{ t+=16; el.style.transform='translateY('+(-t*0.12)+'px) scale('+(1+Math.min(t/500,1)*0.12)+')'; el.style.opacity=1-t/700;
      if(t<700) requestAnimationFrame(anim); else el.remove(); };
  ...
  onEnemyKilled(e){
    ...
    this.coreCharge=Math.min(100,this.coreCharge+(e.def.boss?35:12));
    this.coreReady=this.coreCharge>=100;
    this.updateCoreButton();
  ...
  updateHpBars(){
    ...
    for(const h of board.heroes){
      ...
      const {x,y}=board.worldToScreen(new THREE.Vector3(h.mesh.position.x,h.mesh.position.y+1.48,h.mesh.position.z));
