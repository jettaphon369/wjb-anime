...
class HeroUnit{
  ...
  animate(dtms,board){
    this.t+=dtms;
    const breathe=1+Math.sin(this.t/520+this.seed)*0.009;

    let attackPulse=0;
    if(this.state==='attack'){
      attackPulse=Math.sin(Util.clamp(1-this.stateTimer/220,0,1)*Math.PI);
    }
    const hitPulse=this.state==='hit'?Util.clamp(this.stateTimer/180,0,1):0;

    // Whole-body combat motion works with imported GLB models too.
    const meleeLunge=this.def.melee ? attackPulse*0.17 : -attackPulse*0.045;
    const hitKnock=-hitPulse*0.07;
    ...
    const squash=attackPulse*0.035;
    this.mesh.scale.set(
      breathe*(1+squash),
      breathe*(1-squash*0.7),
      breathe*(1+squash)
    );

    // Smooth shortest-path rotation toward the enemy.
    ...
    this.mesh.rotation.y=this.facingY;
    this.mesh.rotation.z=Math.sin(this.t*.045)*hitPulse*.055;

    if(this.mesh.userData.rightArm) this.mesh.userData.rightArm.rotation.x=-attackPulse*1.15;
    if(this.mesh.userData.leftArm) this.mesh.userData.leftArm.rotation.x=-attackPulse*0.4;
    ...
    setEmissive(this.mesh, hitPulse>0?0xff3b3b:0x000000, hitPulse*0.8);
  }
  animateDeath(dtms){
    if(!this.mesh) return;
...
class EnemyUnit{
  ...
  animate(dtms,board){
    if(!this.mesh) return;
    this.t+=dtms;
    const walkBob=this.state==='walk'?Math.abs(Math.sin(this.t/170+this.seed))*0.045:0;
    const attackKick=this.state==='attack'?Math.sin(Util.clamp(1-this.stateTimer/250,0,1)*Math.PI)*0.10:0;
    this.mesh.position.set(board.worldX(this.col)-attackKick,walkBob,board.worldZ(this.row));
